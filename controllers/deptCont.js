...
const deptCont = {
  ...
  allIndex: async (req, res, next) => {
    try {
      const deptData = await Dept.find().select("name description pic");
  ...
  finder: async (req, res, next) => {
    try {
      Dept.findOne({ name: req.params.name })
        .populate("members")
        .exec((err, foundDept) => {
          if (err) {
            console.log(err);
  ...
  createDept: async (req, res, next) => {
    try {
      ...
      await Dept.findOne({ name }, async (err, existingDept) => {
        if (err) {
          // next(ErrorHandler.serverError());
          return res.redirect("/");
