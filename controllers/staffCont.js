...
const staffCont = {
            ...
            if (!foundKey) {
              ...
              await Staff.findOne({ email }, async (err, staff) => {
                if (err) {
                  ...
                  console.log("Requested Department:", req.body.department);
                  try {
                    await Dept.findOne(
                      ...
                      async (err, foundDept) => {
                        if (err) {
                          console.log(
                            `server Error while verifying department`
                          );
                          next(ErrorHandler.serverError);
                        } else if (!foundDept) {
                          console.log(`Invalid department entered`);
                          req.flash(
                            "error",
                          ...
                          const activationLink = `${process.env.CLIENT_URL}/staff-account/activation/${token}`;
                    ...
                    console.log(err);
                    console.log(`An error occured in dept findone`);
                    req.flash(
                      "error",
          ...
          if (err) {
            ...
            try {
              ...
              await Dept.findOne(
                ...
                async (err, foundDept) => {
                  if (err) {
                    console.log(`server Error while verifying department`);
                    next(ErrorHandler.serverError);
                  } else if (!foundDept) {
                    console.log(`Entered department was invalid`);
                    req.flash("error", "Department is invalid");
                    return res.redirect("/staff-register");
                  } else {
                    await Staff.findOne({ email }, async (err, staff) => {
                      if (err) {
                        ...
                        const savedStaffData = await staff.save();
                        // console.log(`*********************************************`)
                        // console.log(savedStaffData)
                        // console.log(`*********************************************`)
              ...
              console.log(`Error occured in final department validation`);
              req.flash(
                "error",
  ...
  staffLogin: async (req, res, next) => {
    try {
        ...
        // If all checks pass, then the login is successful
        const token = jwt.sign(
            { id: staff._id },
